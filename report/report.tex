...
\paragraph{Cacore}
The private key of the root ca is encrypted and not stored on the system. The root and intermediate CA certificates as well as the private key of the intermediate CA are read only to the main user (cadmin). In case that the intermediate CA's key gets compromised, the intermediate CA certificate can be revoked by the root CA.  The same holds for the issued user certificates and keys. The The MySQL database is restricted to local root access as well remote access for a dedicated user from the caserver only. 
\paragraph{Caserver}
